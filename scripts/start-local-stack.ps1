...
#
#   mongod : a ONE-NODE REPLICA SET on 27018. Transactions need a replica set
#            (see CLAUDE.md); a standalone mongod cannot start a session.
...
# blocks the app.
#
# ASCII only, on purpose: Windows PowerShell 5.1 reads a .ps1 as ANSI unless
# it carries a BOM, so a stray em-dash breaks the parser.
#
...
# rs.initiate() only runs when there is no config yet, so this is idempotent.
$init = "try { rs.status() } catch (e) { rs.initiate({ _id: 'rs0', members: [{ _id: 0, host: '127.0.0.1:27018' }] }) }"
& $MongoSh --port 27018 --quiet --eval $init | Out-Null
...
if (Test-Port 6380) {
  ...
  # --bind 0.0.0.0 so Windows reaches it through WSL's localhost forwarding.
  $cmd = "redis-server --port 6380 --bind 0.0.0.0 --protected-mode no --daemonize yes --save '' --appendonly no"
